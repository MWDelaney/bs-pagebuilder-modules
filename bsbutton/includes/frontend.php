<div class="<?php echo $module->get_classname(); ?>">
	<?php if ( isset($settings->click_action) && $settings->click_action == 'lightbox' ) : ?>
		<a href="<?php echo $settings->lightbox_content_type == 'video' ? $settings->lightbox_video_link : '#'; ?>" class="btn btn-<?php echo ($settings->button_type == "other") ? $settings->button_type_other : $settings->button_type; ?> btn-<?php echo $settings->button_size; ?><?php if('full' == $settings->width): ?> btn-block<?php endif;?> fl-button-lightbox<?php if ('enable' == $settings->icon_animation): ?> fl-button-icon-animation<?php endif; ?>" role="button">
	<?php else : ?>
		<a href="<?php echo $settings->link; ?>" target="<?php echo $settings->link_target; ?>" class="btn btn-<?php echo ($settings->button_type == "other") ? $settings->button_type_other : $settings->button_type; ?> btn-<?php echo $settings->button_size; ?> <?php if('full' == $settings->width): ?> btn-block<?php endif;?><?php if ('enable' == $settings->icon_animation): ?> fl-button-icon-animation<?php endif; ?>" role="button"<?php if ( isset( $settings->link_nofollow ) && 'yes' == $settings->link_nofollow ) echo ' rel="nofollow"'; ?>>
	<?php endif; ?>
